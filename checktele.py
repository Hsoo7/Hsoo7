    ...
    if ispay2[0] == "yes":
        ...
        for i in range(int(msg[0])):
            ...
            if "Available" in isav:
                ...
                try:
                    await hso(functions.channels.UpdateUsernameRequest(
...
async def _(event):
    if ispay2[0] == "yes":
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            username = str(msg[2])
            ch = str(msg[1])
            await event.edit(f"لك يابة هاي صعبة راح احاول اثبت `{username}` على `{ch}` , بعدد `{msg[0]}` من المحاولات !")
...
╚════════════════
    ''')
                ...
                await event.client.send_message(event.chat_id, f"ما مقسوملك  `{username}` ❌❌")
            except Exception as eee:
                await sython.send_message(event.chat_id, f'''خطأ مع {username}
    الخطأ :
    {str(eee)}''')
